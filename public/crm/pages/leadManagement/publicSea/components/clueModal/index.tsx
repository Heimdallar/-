...
import { AutoComplete, Button, Divider, message } from 'poizon-design';
...
import { onSubmit } from '@/usecases/leadManagement/publicSea/clueModal/submit';
import { IListItem, brandObj, Source, ILeadsListObj } from '../../interface';
import { addPublicSeaClue, getBrandByName, queryLabelList, updatePublicSeaClue } from '../../api';
import { defaultItemCol, initFormState, initInternetSaleInfos } from '../../config';
import './index.less';
import { QualificationOptions, applyIntentionOptions } from '../../contant';
...
export default ({
  ...
  const isActivity = row?.source === Source.activity;
  ...
  const [brandOpts, setBrandOpts] = useState<brandObj[]>([]);
  const [brandRet, setBrandRet] = useState<brandObj>();
  ...
  const convertsParams = (params: ILeadsListObj) => {
    const param = params.internetSaleInfos.map((item: ILeadsListObj) => {
      return {
        ...item,
  ...
  return (
    <ModalForm
      ...
      submitter={{
        render: (props) => {
          return [
            <Button key="3" type="primary" onClick={() => submit(props)}>
              {'提交'}
            </Button>,
            <Button
              key="1"
    ...
    >
      ...
      >
        {unchangeableValue?.brandInfo === undefined ? (
          <AutoComplete
            placeholder="请输入品牌名称(例：耐克)，并选择其中一个结果"
      ...
      >
        {unchangeableValue?.mainCategoryInfo === undefined ? (
          <CategorySelect isIdValue placeholder="请选择主营类目" labelInValue />
        ) : (
