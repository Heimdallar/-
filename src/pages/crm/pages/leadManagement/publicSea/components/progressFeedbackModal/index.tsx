...
import fetchClueGetRejectFailsService from '@/services/publicSea/getRejectFails';
import fetchClueGetRejectInvalidLeadsService from '@/services/publicSea/getRejectInvalidLeads';
import fetchClueRejectService from '@/services/publicSea/reject';
import { Feedback, accendantFeedbackOptions, bdFeedbackOptions } from '../../contant';
import {
  StatusEnum,
  ReasonEnum,
  validOptions,
  ...
  FromPage
} from '@/pages/leadManagement/privateSea/contant';
...
const ProgressFeedbackModal: React.FC<Props> = ({
  ...
  const queryRejectFails = async () => {
    const res = await fetchClueGetRejectFailsService({ leadsId });
  ...
  const invokeDetail = async () => {
    const res = await fetchClueDetailService({ leadsId, fromPage: FromPage.公海 });
    if(!res.success) return
